...
if (isset($_POST['submit'])) {
    ...
    if (!empty($array['name_course']) && !empty($array['description']) && !empty($array['tag']) && !empty($array['education_basic']) && !empty($array['field_study']) && !empty($array['type_book']) && !empty($array['name_book']) && !empty($array['lesson'])) {
        ...
        $send = $connection->prepare("INSERT INTO training_courses_mast SET training_courses_name_mast=? , training_courses_teacher_mast=? , training_courses_description_mast=? , training_courses_tag_mast=? , training_courses_education_basic_mast=? , training_courses_field_study_mast=? , training_courses_type_book_mast=? , training_courses_name_book_mast=? , training_courses_lesson_mast=? , training_courses_date_created_course_mast=? , training_courses_date_update_course_mast=?");
        ...
        <div class="container">
            <div class="row">
                <div class="col-12 text-center">
                    <!-- Title -->
                    <h2 class="text-white">افزودن دوره آموزشی</h2>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-8 mx-auto text-center">
                    <!-- Content -->
                    <p class="text-center">از این بخش برای اضافه کردن یک دوره به مدرسه آنلاین TeachLine استفاده کنید . پس از اتمام
                        افزودن دوره ، میتوانید در بخش لیست دوره ها ، دوره مورد نظر را از بخش مدیریت جلسات دوره انتخاب و برای آن ویدیو آموزشی بارگذاری کنید .</p>
                </div>
            </div>
                    ...
                    <form method="post">
                        ...
                        <div style="margin-right: 10% ; margin-left: 10%" class="mb-4">
                            ...
                            <select name="field_study" class="form-select form-select-lg mb-3"
                                    aria-label="Default select example" style="flex-direction: column-reverse;">
                        ...
                        <div style="margin-right: 10% ; margin-left: 10%" class="mb-4">
                            ...
                            <select name="type_book" class="form-select form-select-lg mb-3"
                                    aria-label="Default select example">
                                <?php foreach ($type_book as $type_books) { ?>
                                    <option style="direction: rtl ;text-align: right" value="<?php echo $type_books['type_book_mast'] ?>"><?php echo $type_books['type_book_mast'] ?></option>
                                <?php } ?>
                            </select>
                        </div>
...
<?php if ($success_add) { ?>
    ...
    <script>
        ...
        Toast.fire({
            icon: "success",
            title: "دوره با موفقیت ثبت شد , لطفا منتظر بمانید تا ادمین آنرا تایید و سپس از بخش مدیریت جلسات ویدیو آپلود کنید"
        });
        setTimeout(function () {
...
<?php if ($error_add) { ?>
    '<script>
        ...
        Toast.fire({
            icon: "warning",
            title: "لطفا از پر بودن تمام مقادیر خواسته شده اطمینان حاصل بفرمایید"
        });
    </script>'
<?php } ?>
